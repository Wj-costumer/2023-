...
/*---------------------VARIABLES---------------------*/
float IMU_quat[4] = {0.0f};//互补滤波四元数
float QCS_AHRSq[4]={0};//修正后四元数
/*---------------------FUNCTIONS---------------------*/
/***********************************************************************
** 函 数 名： QCS_Rotate
...
void QCS_GetRollAngel(float Q_input[4],float *Roll)
{
	...
	*Roll = -atan2f(temp_c,(temp_d+1));//反向
}
...
/***********************************************************************
** 函 数 名： QCS_EqSlopeCorrect()
** 函数说明： 误差四元数坡面矫正
**---------------------------------------------------------------------
** 输入参数： 误差四元数、AHRS四元数
** 返回参数： Yaw电机弧度制角度差，Pitch电机弧度制角度差
***********************************************************************/
void QCS_EqSlopeCorrect(float Eq[4],float AHRSq[4],float *EYaw,float *EPitch)
{
	float V_R[3]={1,0,0};
	float V_P[3]={0,1,0};
	float tempQ[4];
	float temp_angel;
	QCS_GetRollAngel(AHRSq,&temp_angel);
	QCS_Rotate(Eq,tempQ,V_R,temp_angel);
	QCS_GetPitchAngel(Eq,EPitch);
	
	QCS_GetPitchAngel(AHRSq,&temp_angel);
	QCS_Rotate(Eq,tempQ,V_P,temp_angel);
	QCS_GetYawAngel(Eq,EYaw);
}

/***********************************************************************
** 函 数 名： QCS_CalculatYP()
** 函数说明： 计算四元数坐标系下Yaw、Pitch云台角度差
**---------------------------------------------------------------------
** 输入参数： 目标四元数、AHRS四元数
** 返回参数： Yaw电机弧度制角度差，Pitch电机弧度制角度差
***********************************************************************/
...
{
	float Eq[4]={0};
	QCS_GetErrorQ(AHRSQ,TargetQ,Eq);
	QCS_EqSlopeCorrect(Eq,AHRSQ,EYaw,EPitch);
}
